...
void execute_command(const char *cmd, ShmBuf *shmp, GtkTextBuffer *text_buffer) {
    if (!GTK_IS_TEXT_BUFFER(text_buffer)) {
        g_warning("Geçersiz text buffer!");
        return;
    }
    ...
    if (pid == -1) {
        ...
        ProcessInfo pinfo;
        pinfo.pid = pid;
        strncpy(pinfo.command, cmd, sizeof(pinfo.command) - 1);
...
void execute_command(const char *cmd, ShmBuf *shmp, GtkTextBuffer *text_buffer) {
    if (!GTK_IS_TEXT_BUFFER(text_buffer)) {
        g_warning("Invalid text buffer for terminal %d", 
                 GPOINTER_TO_INT(g_object_get_data(G_OBJECT(text_buffer), "terminal_id")));
        return;
    }
